...
login-gcloud:
	echo "Logging into GCP using SA."
	gcloud auth activate-service-account --key-file=infra/.gcp.json
...
_%.infra:
	@if [ ! -f .projectid.txt ]; then >&2 echo "No .projectid.txt found" && exit 127; fi
	@if [ ! -f infra/.gcp.json ]; then >&2 echo "No infra/.gcp.json found" && exit 127; fi
